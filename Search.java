...
*******************************************************************************/
// Modifications made by Brandon Jones (Spring 2016) will be marked with 'BJ:'
...
*******************************************************************************/
	...
	private static double fitnessStats[][];  // 0=Avg, 1=Best; BJ: 2=sum2avg, 3=sum2best
	private static int firstPerfectGen[];  // BJ: Keep track of first generation perfect is reached
...
*******************************************************************************/
	...
	//	Set up Fitness Statistics matrix
		...
		firstPerfectGen = new int[Parameters.numRuns+1];  // BJ: (leave first index blank)
		for (int i=0; i<Parameters.generations; i++){
			fitnessStats[0][i] = 0;
			fitnessStats[1][i] = 0;
			fitnessStats[2][i] = 0;  // BJ: Average Fitness Squared
			fitnessStats[3][i] = 0;  // BJ: Best Fitness Squared
		}
	...
	//	an else_if block below to instantiate the problem.
		...
		if (Parameters.problemType.equals("CL")){
			problem = new ComputerLab("test_data1.txt");
		}
		else if (Parameters.problemType.equals("NM")){
	...
	//	Initialize RNG, array sizes and other objects
		...
		bestOfGenChromo = new Chromo();  // BJ, TODO: Create appropriate Chromo subclass
...
//			System.out.println();  // BJ: Skip this line!

			//	Initialize First Generation
			for (int i=0; i<Parameters.popSize; i++){
				member[i] = new Chromo();  // BJ, TODO: Create appropriate Chromo subclass
				child[i] = new Chromo();
			}
				...
				for (int i=0; i<Parameters.popSize; i++){
					...
					if (Parameters.minORmax.equals("max")){
						if (member[i].rawFitness > bestOfGenChromo.rawFitness){
							// BJ, TODO: Use SequentialChromo.copyB2A...
					...
					// BJ: Check if max fitness reached:
					if (firstPerfectGen[R] == 0 &&
							member[i].rawFitness == Parameters.geneSize){
						firstPerfectGen[R] = G;
					}
				}
...
//				System.out.println(R + "\t" + G +  "\t" + (int)bestOfGenChromo.rawFitness + "\t" + averageRawFitness + "\t" + stdevRawFitness);
		...
		// *********************************************************************
			...
			// BJ: Print best of run to screen, and what generation perfect
			// fitness is first achieved (if applicable)
			System.out.println(R + "\t"+ (int)bestOfRunChromo.rawFitness + "\t" + firstPerfectGen[R]);
...
//		}
		...
		// BJ: Enhanced Fitness Statistics matrix (includes std. dev.s)
		summaryOutput.write("Gen                 AvgFit              BestFit             StdevAvg            StdevBest \n");
		for (int i=0; i<Parameters.generations; i++){
			double avgAvg = fitnessStats[0][i]/Parameters.numRuns;
			double avgBest = fitnessStats[1][i]/Parameters.numRuns;
			double stdAvg = Math.sqrt(Math.abs(
					((float) 1/(Parameters.numRuns)) * fitnessStats[2][i] -
					avgAvg*avgAvg
							));
			double stdBest = Math.sqrt(Math.abs(
					((float) 1/(Parameters.numRuns)) * fitnessStats[3][i] -
					avgBest*avgBest
							));
			Hwrite.left(i, 15, summaryOutput);
			Hwrite.left(avgAvg, 20, 2, summaryOutput);
			Hwrite.left(avgBest, 20, 2, summaryOutput);
			Hwrite.left(stdAvg, 20, 2, summaryOutput);
			Hwrite.left(stdBest, 20, 2, summaryOutput);
			summaryOutput.write("\n");
		}
