//This code was originally written by Brandon Jones
//Modified by Alex Consunji
...
public class ComputerLabBinary extends FitnessFunction{
...
*******************************************************************************/
	...
	public ComputerLabBinary(String availability_file){
...
//  COMPUTE SIMPLE RAW FITNESS **********************************************************
	...
	public void doSimpleRawFitness(Chromo X){
		...
		for (int a = 0; a < Parameters.geneSize; a++){
			//convert binary string to int for easy sorting and set the last bit to
			int assignment = Integer.parseInt(X.chromo.get(a), 2)-1;
			int person = a / 5;
...
//  COMPUTE A CHROMOSOME'S RAW FITNESS USING A STEP FUNCTION ****************************
	...
	public void doRawFitnessStep(Chromo X){
		...
		// Check each assignment:
		for (int a = 0; a < Parameters.geneSize; a++){
			//convert binary string to int for easy sorting
			int assignment = Integer.parseInt(X.chromo.get(a), 2) - 1;
			int person = a / 7;
			...
			if (this.availability[person][assignment] == 0){
				...
				X.rawFitness += 2;  // Available, but not top 3
			else if (this.availability[person][assignment] == 3)
				X.rawFitness += 3;  // Top 3 choices
			else if (this.availability[person][assignment] == 2)
				X.rawFitness += 3;  // Top 3 choices
			else if (this.availability[person][assignment] == 1)
				X.rawFitness += 4;  // Top 3 choices
